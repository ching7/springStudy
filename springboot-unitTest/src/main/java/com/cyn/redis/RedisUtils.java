...
public class RedisUtils {
    ...
    private static volatile RedisUtils obj;
...
//	}
     ...
     */
    public static RedisUtils getInstance() throws Exception {
        if (null == obj) {
            synchronized (RedisUtils.class) {
                if (null == obj) {
                    StringRedisTemplate stringRedisTemplate = SpringBeanUtils.getBean(StringRedisTemplate.class);
                    if (null == stringRedisTemplate) {
                        throw new Exception("SpringContext can not find stringRedisTemplate");
                    }
                    obj = new RedisUtils();
                    obj.redisTemplate = stringRedisTemplate;
                }
            }
        }
        return obj;
     ...
     * @param key   the key
     * @param value the value
     * @return 之前已经存在返回false, 不存在返回true if absent
     */
     ...
     *
     * @param maps the maps
     * @return 之前已经存在返回false, 不存在返回true boolean
     */
